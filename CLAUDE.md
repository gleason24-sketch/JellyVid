...
A production AI video studio on the Higgsfield API, built so a non-technical
creator goes from landing page to first generation in under 60 seconds.

**The hook is the face.** Seedance 2.5 reference-to-video accepts real face
inputs and is available in the US — a capability the Higgsfield API has and
other providers do not. So the product is *put yourself in the movie*: upload a
selfie, tap a scene, star in it. Everything else is in service of that being
fast, shareable, and honest about money.

The competing open-source studio is a bring-your-own-key power tool with 32
models behind one prompt bar. We do not compete on model count. We compete on a
person with no API key and no prompting skill getting something they want to
send to a friend, in about a minute. Optimise every decision for that.
...
| 6 | Convoluted carnival UI | Five task cards, one featured; model chosen for you; scene presets mean you need not write a prompt at all | `src/lib/models.ts`, `src/components/studio.tsx` |
...
- Likeness inputs require the consent checkbox; the timestamp is stored on the
  job row. Obvious public figures are refused before any credit is spent. Any
  task whose `input` is not `none` is treated as a likeness, without exception.
- Reference-to-video takes `image_urls` (1–30) and **requires an explicit
